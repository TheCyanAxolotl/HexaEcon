            ...
            if (args.size >= 1) {
                        ...
                        try {
                            if (args.size < 2) {
                                ...
                                if (!(Format.hasLetter(args[1]) || Format.hasLetterAndSpecial(args[1]) || Format.hasLetterAndMabyeDigit(args[1]) || Format.hasLetterSpecialAndMaybeDigit(args[1]) || Format.hasSpecial(args[1]) || Format.hasLetter(args[1]))) {
                                    ...
                                    if (remove >= 1) {
                                        val data_names_sqlite = File(plugin.dataFolder.toString() + "/data/${sender.name}/" + sender.name + "_SQLite.txt")
                                        val data_names_mysql = File(plugin.dataFolder.toString() + "/data/${sender.name}/" + sender.name + "_MySQL.txt")
                                        val data_names_mongodb = File(plugin.dataFolder.toString() + "/data/${sender.name}/" + sender.name + "_MongoDB.txt")
                                        val data_names_redis = File(plugin.dataFolder.toString() + "/data/${sender.name}/" + sender.name + "_Redis.txt")
                                        ...
                                        val somasqlite = data_names_config_sqlite.getLong("data.${dataeconomyvalue}") - remove
                                        val somamysql = data_names_config_mysql.getLong("data.${dataeconomyvalue}") - remove
                                        val somamongodb = data_names_config_mongodb.getLong("data.${dataeconomyvalue}") - remove
                                        val somaredis = data_names_config_redis.getLong("data.${dataeconomyvalue}") - remove
                                        if (databasetype == "h2") {
                                            if (data_names_config_sqlite.getLong("data.${dataeconomyvalue}") >= remove) {
                                                ...
                                                (sender as Player).inventory.addItem(Economy.addEconomy(sender, remove.toInt()))
                                            } else {
                ...
                if (args[0] == "remove") {
                    if (player.hasPermission("hexaecon.permissions.remove")) {
                            ...
                            if (args.size < 2) {
                                ...
                                if (!(Format.hasLetter(args[1]) || Format.hasLetterAndSpecial(args[1]) || Format.hasLetterAndMabyeDigit(args[1]) || Format.hasLetterSpecialAndMaybeDigit(args[1]) || Format.hasSpecial(args[1]) || Format.hasLetter(args[1]))) {
                                    ...
                                    if (remove >= 1 && args.size >= 3) {
                                        ...
                                        if (targetname.isEmpty() || targetname.isBlank()) {
                                        ...
                                        val data_names_sqlite = File(plugin.dataFolder.toString() + "/data/${target!!.name}/" + target.name + "_SQLite.txt")
                                        val data_names_mysql = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_MySQL.txt")
                                        val data_names_mongodb = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_MongoDB.txt")
                                        val data_names_redis = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_Redis.txt")
                                        val data_names_config_sqlite: FileConfiguration = YamlConfiguration.loadConfiguration(data_names_sqlite)
                                        val data_names_config_mysql: FileConfiguration = YamlConfiguration.loadConfiguration(data_names_mysql)
                                        val data_names_config_mongodb: FileConfiguration = YamlConfiguration.loadConfiguration(data_names_mongodb)
                                        val data_names_config_redis: FileConfiguration = YamlConfiguration.loadConfiguration(data_names_redis)
                                        ...
                                        if (databasetype == "h2") {
                                            if (data_names_config_sqlite.getLong("data.${dataeconomyvalue}") >= remove) {
                                                ...
                                                sender.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(removedEconFromPlayer().replace("%amount%", remove.toString()).replace("%valuename%", dataeconomyvalue).replace("%p%", target.name).replace("%amountformatted%", Economy.formatBalance(remove.toString()))))))
                                                plugin.reloadConfig()
                                            } else {
                                                sender.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(cannotRemoveEconFromPlayer().replace("%amount%", remove.toString()).replace("%amountformatted%", Economy.formatBalance(remove.toString())).replace("%valuename%", dataeconomyvalue).replace("%p%", target.name).replace("%targetbalance%", data_names_config_sqlite.getLong("data.${dataeconomyvalue}").toString())))))
                                            }
                                        } else if (databasetype == "MongoDB") {
                                            if (data_names_config_mongodb.getLong("data.${dataeconomyvalue}") >= remove) {
                                                try {
                                                ...
                                                sender.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(removedEconFromPlayer().replace("%amount%", remove.toString()).replace("%amountformatted%", Economy.formatBalance(remove.toString())).replace("%valuename%", dataeconomyvalue).replace("%p%", target.name)))))
                                                plugin.reloadConfig()
                                            } else {
                                                sender.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(cannotRemoveEconFromPlayer().replace("%amount%", remove.toString()).replace("%amountformatted%", Economy.formatBalance(remove.toString())).replace("%valuename%", dataeconomyvalue).replace("%p%", target.name).replace("%targetbalance%", data_names_config_mongodb.getLong("data.${dataeconomyvalue}").toString())))))
                                            }
                                        } else if (databasetype == "MySQL") {
                                            if (data_names_config_mysql.getLong("data.$dataeconomyvalue") >= remove) {
                                                try {
                                                ...
                                                sender.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(cannotRemoveEconFromPlayer().replace("%amount%", remove.toString()).replace("%amountformatted%", Economy.formatBalance(remove.toString())).replace("%valuename%", dataeconomyvalue).replace("%p%", target.name).replace("%targetbalance%", data_names_config_mysql.getLong("data.$dataeconomyvalue").toString())))))
                                            }
                                        } else if (databasetype == "Redis") {
                                            if (data_names_config_redis.getLong("data.${dataeconomyvalue}") >= remove) {
                                                try {
                        ...
                        if (soundnoperm != "NONE") {
                            (player as Player).player?.playSound(player.location, Sound.valueOf(soundnoperm), volumenoperm.toFloat(), pitch.toFloat())
                        }
                        player.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(accessDenied().replace("%perm%", "hexaecon.permissions.remove")))))
                    }
                }
                if (args[0] == "generate") {
                            ...
                            if (args.size < 2) {
                                ...
                                if (args.size < 3) {
                                    ...
                                    try {
                                        ...
                                        val data_names2_sqlite = File(plugin.dataFolder.toString() + "/data/${target!!.name}/" + target.name + "_SQLite.txt")
                                        val data_names2_mysql = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_MySQL.txt")
                                        val data_names2_mongodb = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_MongoDB.txt")
                                        val data_names2_redis = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_Redis.txt")
                                        val data_names_config2_sqlite: FileConfiguration = YamlConfiguration.loadConfiguration(data_names2_sqlite)
                                        val data_names_config2_mysql: FileConfiguration = YamlConfiguration.loadConfiguration(data_names2_mysql)
                                        val data_names_config2_mongodb: FileConfiguration = YamlConfiguration.loadConfiguration(data_names2_mongodb)
                                        val data_names_config2_redis: FileConfiguration = YamlConfiguration.loadConfiguration(data_names2_redis)
                                        val data_names_sqlite = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_SQLite.txt")
                                        val data_names_mysql = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_MySQL.txt")
                                        val data_names_mongodb = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_MongoDB.txt")
                                        val data_names_redis = File(plugin.dataFolder.toString() + "/data/${target.name}/" + target.name + "_Redis.txt")
                                        val somasqlite = data_names_config2_sqlite.getLong("data.${dataeconomyvalue}") + amount
                                        val somamysql = data_names_config2_mysql.getLong("data.${dataeconomyvalue}") + amount
                                        val somamongodb = data_names_config2_mongodb.getLong("data.${dataeconomyvalue}") + amount
                                        val somaredis = data_names_config2_redis.getLong("data.${dataeconomyvalue}") + amount
                                        try {
                                            when (databasetype) {
                                                "h2" -> {
                                                    TableFunctionSQL.dropTableSQLite(target)
                                        ...
                                        plugin.reloadConfig()
                                        target.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(generateToOther().replace("%amount%", amount.toString()).replace("%valuename%", dataeconomyvalue).replace("%p%", player.name).replace("%amountformatted%", Economy.formatBalance(amount.toString()))))))
                                        return true
                                    } catch (e: Exception) {
                                        player.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(invalidAmount().replace("%valuename%", dataeconomyvalue)))))
                                        e.printStackTrace()
                ...
                if (args[0] == "set") {
                    if (player.hasPermission("hexaecon.permissions.set")) {
                            ...
                            if (args.size < 2) {
                                ...
                                if (args.size < 3) {
                                    ...
                                    try {
                                        ...
                                        plugin.reloadConfig()
                                        target.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(setToOther().replace("%amount%", amount.toString()).replace("%valuename%", dataeconomyvalue).replace("%p%", player.name).replace("%amountformatted%", Economy.formatBalance(amount.toString()))))))
                                        return true
                                    } catch (e: Exception) {
                                        player.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(invalidAmount().replace("%valuename%", dataeconomyvalue)))))
                                        e.printStackTrace()
                        ...
                        player.sendMessage(Format.hex(Format.color(IridiumColorAPI.process(accessDenied().replace("%perm%", "hexaecon.permissions.set")))))
                    }
                }
            }
        }
